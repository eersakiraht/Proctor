    ...
    try:
        ...
        if os.path.exists(obj_model_path):
            obj_opts = mp_vision.ObjectDetectorOptions(
                ...
                score_threshold=PHONE_DETECTION_CONFIDENCE,
                category_allowlist=["cell phone", "phone", "book"]
            )
            _mp_object_detector = mp_vision.ObjectDetector.create_from_options(obj_opts)
            print(f"[INFO] MediaPipe Tasks ObjectDetector initialised (threshold={PHONE_DETECTION_CONFIDENCE}).")
...
def classify_gaze(landmarks, yaw: float, pitch: float) -> str:
    """
    Returns CENTER / LEFT / RIGHT / UP / DOWN based on head pose + iris position.
    Calibrated for professional exam proctoring:
    - Looking directly at screen or looking into the webcam lens is strictly CENTER.
    - Looking UP detects head tilting up (pitch > 16 deg) or compound eye glance up.
    - Looking DOWN detects head tilting down (pitch < -15 deg) or compound eye glance down at desk/lap/phone.
    - Looking LEFT/RIGHT detects head turn (yaw > 20 deg or < -20 deg) or strong eye glance.
    """
    ...
    if len(landmarks) > 473:
        try:
            # Vertical gaze tracking (eyelids vs pupil center)
            ...
            if l_height > 0.005 and r_height > 0.005:
                l_v_ratio = (l_iris_y - min(l_top, l_bot)) / l_height
                r_v_ratio = (r_iris_y - min(r_top, r_bot)) / r_height
                avg_v_ratio = (l_v_ratio + r_v_ratio) / 2.0

            # Horizontal gaze tracking (eye corners vs pupil center)
            left_outer = landmarks[33].x
            left_inner = landmarks[133].x
            left_iris  = landmarks[468].x
            eye_width  = abs(left_inner - left_outer)
            if eye_width > 0.001:
                h_ratio = (left_iris - min(left_outer, left_inner)) / eye_width
        except Exception:
            pass

    # 2. Horizontal iris glance while head is facing forward
    if h_ratio is not None:
        if h_ratio < IRIS_HORIZONTAL_RIGHT_RATIO:
            return "RIGHT"
        elif h_ratio > IRIS_HORIZONTAL_LEFT_RATIO:
            return "LEFT"
    ...
    # Catches eye glances up or down even with slight/subtle head movement
    if avg_v_ratio is not None:
        # Looking DOWN at desk/notes/phone:
        # Either head slightly down (<= -9 deg) + eyes down (> 0.55), or eyes cast down (> 0.65)
        if (pitch <= -9.0 and avg_v_ratio > 0.55) or (avg_v_ratio > IRIS_VERTICAL_DOWN_RATIO and pitch <= 4.0):
            return "DOWN"

        # Looking UP at ceiling:
        # Either head slightly up (>= 9 deg) + eyes up (< 0.30), or eyes rolled up (< 0.24)
        if (pitch >= 9.0 and avg_v_ratio < 0.30) or (avg_v_ratio < IRIS_VERTICAL_UP_RATIO and pitch >= -2.0):
            return "UP"
...
def analyze_frame(base64_frame: str) -> FrameAnalysisResponse:
    ...
    # ---- Phase 5b: Phone & Prohibited Object Detection --------------------
        ...
        try:
            ...
            if obj_result and obj_result.detections:
                for det in obj_result.detections:
                    # Validate bounding box to prevent noise / whole-frame false positives
                    bbox = det.bounding_box
                    box_w = bbox.width
                    box_h = bbox.height
                    box_area = (box_w * box_h) / float(w * h)

                    # A real mobile phone held in webcam field of view occupies between 1.5% and 55% of the frame
                    if box_area < 0.015 or box_area > 0.55:
                        continue

                    aspect = float(box_w) / max(1.0, float(box_h))
                    # Mobile phone in portrait or landscape: aspect ratio between 0.30 and 3.2
                    if aspect < 0.30 or aspect > 3.2:
                        continue

                    for cat in det.categories:
                        score = float(cat.score)
                        if score < PHONE_DETECTION_CONFIDENCE:
                            continue
                        ...
                        if "phone" in cname or "cell" in cname:
                            ...
                            break
                        elif "book" in cname and score >= 0.65:
                            events.append(EventItem(
