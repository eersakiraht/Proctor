...
public class ReportExportService {
    ...
    public byte[] exportPdf(Long attemptId) {
        ...
        try {
            ...
            // Title
            doc.add(new com.lowagie.text.Paragraph("Proctoring Report", titleFont));
            doc.add(com.lowagie.text.Chunk.NEWLINE);

            // Summary table
            doc.add(new com.lowagie.text.Paragraph("Summary", sectionFont));
            com.lowagie.text.pdf.PdfPTable summaryTable = new com.lowagie.text.pdf.PdfPTable(2);
            summaryTable.setWidthPercentage(100);
            addPdfRow(summaryTable, "Attempt ID", String.valueOf(ctx.attempt.getId()));
            addPdfRow(summaryTable, "Student", ctx.attempt.getStudentEmail());
            addPdfRow(summaryTable, "Exam", ctx.attempt.getExam().getName());
            addPdfRow(summaryTable, "Status", ctx.attempt.getStatus());
            addPdfRow(summaryTable, "Score", nullSafe(ctx.attempt.getScore()));
            addPdfRow(summaryTable, "Flagged for Review", String.valueOf(Boolean.TRUE.equals(ctx.attempt.getFlaggedForReview())));
            addPdfRow(summaryTable, "Risk Score", ctx.riskScore.toPlainString());
            addPdfRow(summaryTable, "Start Time", format(ctx.attempt.getStartTime()));
            addPdfRow(summaryTable, "End Time", format(ctx.attempt.getEndTime()));
