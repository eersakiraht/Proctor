...
export default function ExamTake() {
  ...
  // Strike & gaze tracking refs
  const strikesRef = useRef<number>(0);
  const lookAwayStartRef = useRef<number | null>(null);
  const lookAwayWarnedRef = useRef<boolean>(false);
  const lastStrikeTimeRef = useRef<number>(0);
  const consecutivePhoneDetections = useRef<number>(0);
  const consecutiveMultiFaceDetections = useRef<number>(0);
  const lastPhoneStrikeTime = useRef<number>(0);
  const lastMultiFaceStrikeTime = useRef<number>(0);
  const lastGazeStrikeTime = useRef<number>(0);
  const lastVoiceStrikeTime = useRef<number>(0);
  ...
  // Keep video element synchronized with active webcam stream
  useEffect(() => {
        ...
        try {
          ...
          if (ctx) {
            ...
            if (isMounted && res) {
              ...
              // Gaze & Face presence tracking (Sustained look-away for >= 2 seconds)
              ...
              if (isLookingAway) {
                if (!lookAwayStartRef.current) {
                  ...
                  if (elapsedMs >= 2000 && !lookAwayWarnedRef.current) {
                    const now = Date.now();
                    if (now - lastGazeStrikeTime.current >= 8000) {
                      lookAwayWarnedRef.current = true;
                      lastGazeStrikeTime.current = now;
                      const dirText = res.gazeDirection !== "CENTER" ? `looking ${res.gazeDirection.toLowerCase()}` : "face not detected";
                      issueWarningStrike(
                        `Looking Away (${res.gazeDirection || "AWAY"}) for 2+ Seconds`,
                        `Continuous gaze infraction detected (${dirText} for ${(elapsedMs / 1000).toFixed(1)}s). Please look directly at the examination screen or camera.`
                      );
                    }
                  }
                }
                ...
                lookAwayWarnedRef.current = false;
              }

              // Multi-face detection with 2-frame persistence to avoid single-frame false positives
              if (res.faceCount > 1) {
                consecutiveMultiFaceDetections.current++;
                if (consecutiveMultiFaceDetections.current >= 2) {
                  const now = Date.now();
                  if (now - lastMultiFaceStrikeTime.current >= 8000) {
                    lastMultiFaceStrikeTime.current = now;
                    issueWarningStrike(
                      "Multiple Faces Detected",
                      "More than one face was consistently detected in the camera frame."
                    );
                  }
                }
              } else {
                consecutiveMultiFaceDetections.current = 0;
              }

              // Mobile phone detection with 2-frame persistence to eliminate single-frame glitches
              const hasPhone = res.phoneDetected || res.events?.some((e) => e.type === "CELL_PHONE_DETECTED");
              if (hasPhone) {
                consecutivePhoneDetections.current++;
                if (consecutivePhoneDetections.current >= 2) {
                  const now = Date.now();
                  if (now - lastPhoneStrikeTime.current >= 8000) {
                    lastPhoneStrikeTime.current = now;
                    issueWarningStrike(
                      "Mobile Phone Detected",
                      "Unauthorized mobile device detected in camera frame."
                    );
                  }
                }
              } else {
                consecutivePhoneDetections.current = 0;
              }
            }
          }
        } catch {
  ...
  useEffect(() => {
    ...
    async function initAudioDetection() {
      try {
        ...
        // Calculate bin boundaries with 30 Hz minimum frequency
        const minBin30Hz = Math.max(1, Math.floor(30 / binResolution)); // Starts strictly at 30 Hz
        const lowBandEnd = Math.min(bufferLength - 1, Math.ceil(250 / binResolution)); // 30 Hz - 250 Hz (sub-bass, low hums, deep voices)
        const vocalBandEnd = Math.min(bufferLength - 1, Math.ceil(3500 / binResolution)); // 30 Hz - 3500 Hz (full speech & murmur spectrum)

        console.log(`[VoiceDetection] Listening for voice (30Hz+ minBin=${minBin30Hz} binRes=${binResolution.toFixed(1)}Hz)`);
        ...
        const checkAudio = () => {
          ...
          // 2. Frequency-domain analysis starting from 30 Hz minimum:
          // A) Low frequency band: 30 Hz to 250 Hz
          let lowFreqSum = 0;
          let maxLowFreq = 0;
          let lowCount = 0;
          for (let i = minBin30Hz; i <= lowBandEnd; i++) {
          ...
          // B) Full speech & murmur band: 30 Hz to 3500 Hz
          let vocalSum = 0;
          let maxVocal = 0;
          let vocalCount = 0;
          for (let i = minBin30Hz; i <= vocalBandEnd; i++) {
            const val = dataArray[i];
            vocalSum += val;
          ...
          // Highly sensitive sound detection starting from 30 Hz minimum:
          ...
          if (isSoundDetected) {
            ...
            if (!voiceStartRef) {
              ...
              if (elapsed >= 2000 && !voiceWarnedRef) {
                if (now - lastVoiceStrikeTime.current >= 6000) {
                  voiceWarnedRef = true;
                  lastVoiceStrikeTime.current = now;
                  logEvent("VOICE_DETECTED", undefined, {
                    durationSeconds: Math.round(elapsed / 1000),
                    reason: "Continuous sound or voice (30Hz+) detected for 2+ seconds",
                  });
                  flushNow();
                  issueWarningStrike(
                    "Voice/Sound Detected",
                    "Continuous sound or voice (30Hz+) detected for 2+ seconds. Exam environment must remain quiet."
                  );
                }
              }
            }
    ...
    function onFullscreenChange() {
      if (!document.fullscreenElement) {
        ...
        issueWarningStrike(
          ...
          `You exited fullscreen mode (exit #${fullscreenViolationCount.current}). All examination activity must remain in fullscreen.`
        );
          ...
          {cameraMinimized ? (
            ...
            <div className="w-64 sm:w-72 bg-slate-900/95 text-white rounded-xl border border-slate-700/80 overflow-hidden shadow-2xl backdrop-blur-md">
              ...
              <div className="relative aspect-video bg-black flex items-center justify-center overflow-hidden">
                ...
                {/* Real-time AI Face & Gaze Detection Overlay */}
                <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between pointer-events-none">
                  {aiAnalysis ? (
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full backdrop-blur-md border ${
                        aiAnalysis.phoneDetected || aiAnalysis.events?.some((e) => e.type === "CELL_PHONE_DETECTED")
